...
var _netwizard_es = __webpack_require__(330);

var _netwizard_es2 = _interopRequireDefault(_netwizard_es);
...
var _animate = __webpack_require__(332);
...
var Netwizard = function (_mix) {
    ...
    _createClass(Netwizard, [{
        ...
        value: function addListener() {
            ...
            this.on(this.prevBtn, 'click', function () {
...
var Create = function () {
	...
	_createClass(Create, [{
		...
		key: 'createStep',
		value: function createStep(data) {
			var step = '<div  class="stepBar" >\n                \t\t<ul>';
...
var Animate = function () {
	...
	_createClass(Animate, [{
		key: 'slideShow',
		value: function slideShow(el, effect, other) {
...
var Common = function () {
	...
	_createClass(Common, [{
		...
		// 获取元素的某个样式
		value: function getCss(el, styleName) {
			// 判断是否为IE浏览器
		...
		key: 'converseNameStyle',
		value: function converseNameStyle(name) {
			var type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;

			name = type ? name.replace(/-\w/g, function (v) {
				return v.replace('-', '').toUpperCase();
			}) : name.replace(/[A-Z]/g, function (v) {
		...
		key: 'setCss',
		value: function setCss(el, css) {
			var _iteratorNormalCompletion = true;
			var _didIteratorError = false;
			var _iteratorError = undefined;

			try {
				for (var _iterator = Object.entries(css)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
		...
		value: function ajax() {
			...
			try {
				for (var _iterator2 = Object.entries(opt.data)[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
			...
			var postData = [].concat(_toConsumableArray(params)).join('&');
			if (opt.method === 'POST') {
...
var Base = function () {
    ...
    _createClass(Base, [{
        ...
        value: function switchStep(direction) {
            ...
            this.select('#footer_btn .next .control-detail').innerText = config.stepdetail['step' + (config.curStep + 1)] ? config.stepdetail['step' + (config.curStep + 1)].stepText : '';
            this.select('#footer_btn .prev .control-detail').innerText = config.stepdetail['step' + (config.curStep - 1)] ? config.stepdetail['step' + (config.curStep - 1)].stepText : '';
...
var SaveData = function () {
    ...
    _createClass(SaveData, [{
        ...
        value: function saveData() {
            var self = this;
            var lastData = self.interfaceConfig.stepdetail['step' + self.interfaceConfig.curStep];
            var save = {
                step1: function step1() {
                ...
                step3: function step3() {
                    var contentData = lastData.options;
                    for (var i = 0; i < contentData.length; i++) {
                        if (contentData[i].type === 'text') {
        ...
        key: 'saveRadio',
        value: function saveRadio(lastData) {
            var checkedRadio = this.getChecked(lastData.name);
