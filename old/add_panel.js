...
RuleAddPanel.prototype.init_panel_config = function(panel_config) {
    ...
    /*检查输入正确与否的工具*/
    // 优化：通过这种方式避免了继承时构造函数的原型对象会执行两遍的问题，并且修改子类方法同时修改父类方法的问题
    rule_add_panel.check_tool.prototype = Object.create(ChinArk_forms.prototype);
    rule_add_panel.check_tool.prototype.constructor = ChinArk_forms
}
...
RuleAddPanel.prototype.load_data_into_add_panel = function(data_item) {
    ...
    form_selects.each(function() {
        ...
        if(data_item[select.name] == null){
            data_item[select.name] = '';
        }
